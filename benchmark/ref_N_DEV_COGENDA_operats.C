...
void Traits::loadInstanceParameters(ParametricData<COGENDAoperats::Instance> &p)
{
  ...
  // no "TEMP".
  p.addPar("XYCEADMSINSTTEMP", 0.0, &COGENDAoperats::Instance::cogendaInstTemp)
    .setExpressionAccess(NO_DOC)
    .setUnit(U_DEGK)
    .setCategory(CAT_TEMP)
    .setDescription("Internally set parameter for device instance temperature");
  p.addPar("CJO",static_cast<double>(9.999E-12), &COGENDAoperats::Instance::CJO);
  p.addPar("IS",static_cast<double>(1.0e-14), &COGENDAoperats::Instance::IS);
}

  //void Traits::loadModelParameters()
void Traits::loadModelParameters(ParametricData<COGENDAoperats::Model> &p)
{
  p.addPar("XYCEADMSMODTEMP", 0.0, &COGENDAoperats::Model::cogendaModTemp)
    .setExpressionAccess(NO_DOC)
    .setUnit(U_DEGK)
    .setCategory(CAT_TEMP)
    .setDescription("Internally set parameter for device model temperature");
  p.addPar("CJO",static_cast<double>(9.999E-12), &COGENDAoperats::Model::CJO);
  p.addPar("IS",static_cast<double>(1.0e-14), &COGENDAoperats::Model::IS);
}
...
{
  vnex = ((cogenda_ternary_op<CogendaFadType>((vdio > 2.1),(eww/vdio),(vdio+10.3)))*UNITFAD).val();
  j = ((12)*UNITFAD).val();
  qdio = ((pow(vdio,(eww/4.5)))*UNITFAD).val();
  qdio = ((log((3.4*(6.7-(7.5/3.0)))))*UNITFAD).val();
  if((!(vdio < 0.5) && (qdio > 0))) {
    {
      vnew = ((cogenda_ternary_op<CogendaFadType>((vdio > 0.1),(eww/vdio),((1.2*vdio)+10.3)))*UNITFAD).val();
      vdio = (((((-vdio*2)%5)+(vnew*pow(vdio,2))))*UNITFAD).val();
...
void registerDevice()
{
  Config<Traits>::addConfiguration()
    .registerDevice("operats", 1)
    .registerModelType("operats", 1);
}
} // namespace COGENDAoperats
} // namespace Device
} // namespace Xyce
...
struct Bootstrap {
 Bootstrap() {
  Xyce::Device::COGENDAoperats::registerDevice(); }}; 
Bootstrap COGENDAoperats_bootstrap;
