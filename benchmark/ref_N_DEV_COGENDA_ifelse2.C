...
void Traits::loadInstanceParameters(ParametricData<COGENDAifelse2::Instance> &p)
{
  ...
  // no "TEMP".
  p.addPar("XYCEADMSINSTTEMP", 0.0, &COGENDAifelse2::Instance::cogendaInstTemp)
    .setExpressionAccess(NO_DOC)
    .setUnit(U_DEGK)
    .setCategory(CAT_TEMP)
    .setDescription("Internally set parameter for device instance temperature");
  p.addPar("CJO",static_cast<double>(9.999E-12), &COGENDAifelse2::Instance::CJO);
  p.addPar("IS",static_cast<double>(1.0e-14), &COGENDAifelse2::Instance::IS);
}

  //void Traits::loadModelParameters()
void Traits::loadModelParameters(ParametricData<COGENDAifelse2::Model> &p)
{
  p.addPar("XYCEADMSMODTEMP", 0.0, &COGENDAifelse2::Model::cogendaModTemp)
    .setExpressionAccess(NO_DOC)
    .setUnit(U_DEGK)
    .setCategory(CAT_TEMP)
    .setDescription("Internally set parameter for device model temperature");
  p.addPar("CJO",static_cast<double>(9.999E-12), &COGENDAifelse2::Model::CJO);
  p.addPar("IS",static_cast<double>(1.0e-14), &COGENDAifelse2::Model::IS);
}
  ...
  if((vdio < 0.5)) {
    {
      if((vnew > 0)) {
        ...
        qdio = ((((-0.5*this->CJO)*(1-sqrt((vdio-0.6)))))*UNITFAD).val();
      }
    }
  }
  else if((vdio < 1.0)) {
    {
      qdio = ((((1.0*this->CJO)*(1-sqrt((vdio-0.6)))))*UNITFAD).val();
    }
  }
  else if((vdio > 1.2)) {
    {
  ...
  else {
    {
      if((qdio > 3e-15)) {
        ...
        {
          xix = (((Ic0/I00))*UNITFAD).val();
          qdio = (((this->CJO*xix))*UNITFAD).val();
        }
      }
    }
  }
...
void registerDevice()
{
  Config<Traits>::addConfiguration()
    .registerDevice("ifelse2", 1)
    .registerModelType("ifelse2", 1);
}
} // namespace COGENDAifelse2
} // namespace Device
} // namespace Xyce
...
struct Bootstrap {
 Bootstrap() {
  Xyce::Device::COGENDAifelse2::registerDevice(); }}; 
Bootstrap COGENDAifelse2_bootstrap;
